...
This page documents the HTTP headers used by the Grow Team API.
...
- For `Basic` authentication credentials in the Grow Team API, a "username"
...
If provided, the Grow Team server will parse the `User-Agent` HTTP header
...
Official Grow Team clients and integrations use a `User-Agent` that starts
with something like `ZulipMobile/20.0.103 `, encoding the name of the
application and it's version.

Grow Team's official API bindings have reasonable defaults for
`User-Agent`. For example, the official Grow Team Python bindings have a
...
official Grow Team Nagios integration is initialized like this:
...
[#integrations][integrations-channel] in the [Grow Team development
community](https://zulip.com/development-community/).

## Rate-limiting response headers

To help clients avoid exceeding rate limits, Grow Team sets the following
...
[Grow Team's rate limiting rules are configurable][rate-limiting-rules],
...
When the Grow Team server has configured multiple rate limits that apply
