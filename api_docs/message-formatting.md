...
Grow Team supports an extended version of Markdown for messages, as well as
some HTML level special behavior. The Grow Team help center article on [message
formatting](/help/format-your-message-using-markdown) is the primary
documentation for Grow Team's markup features. This article is currently a
...
**Changes**: As of Grow Team 4.0 (feature level 33), [code blocks][help-code]
...
**Changes**: In Grow Team 12.0 (feature level 451), invalid timestamp formats
...
**Changes**: In Grow Team 3.0 (feature level 8), added [global time
mentions][help-global-time] to supported Markdown message formatting
features.

## Links to channels, topics, and messages

Grow Team's markup supports special readable Markdown syntax for [linking
to channels, topics, and
messages](/help/link-to-a-message-or-conversation). See also [Grow Team
URLs](/api/zulip-urls)
...
**Changes**: In Grow Team 11.0 (feature level 400), the server switched
...
Before Grow Team 10.0 (feature level 347), the `with` field
...
Grow Team's [emoji][help-emoji] support includes standard Unicode emoji, a
built-in Grow Team custom emoji like `:zulip:` and [custom realm
...
size. For example, Grow Team the web app scales large emoji to be 2x the
...
Note that Unicode emoji in messages that were originally sent on Grow Team
versions older than Grow Team 1.9.2 will not have the `role` or
...
the Grow Team server's hostname:
...
**Changes**: Large emoji are new in Grow Team 12.0 (feature level 436).

## Images

When a Grow Team message is sent with a link to an uploaded image, Grow Team will
...
When a Grow Team message is sent referencing an **uploaded image** in Markdown
image syntax, (e.g.,`![example image](/user_uploads/path/to/example.png)`),
Grow Team will generate an image element with the following format:
...
Previously, with the exception of Grow Team servers that used the beta
Thumbor-based implementation years ago, all image previews in Grow Team
...
When a Grow Team message is sent linking to an uploaded video, Grow Team may
...
If the Grow Team server has rewritten the URL of the audio file, it will
provide the URL in a `data-original-url` parameter. The Grow Team server
...
The Grow Team server does not validate whether uploaded files with an
audio `Content-Type` are actually playable.

**Changes**: New in Grow Team 11.0 (feature level 405).

## Mentions and silent mentions

Grow Team markup supports [mentioning](/help/mention-a-user-or-group)
...
In Grow Team 11.0 (feature level 395), the Grow Team server stopped generating
legacy Dropbox link previews. Dropbox links are now previewed just
like standard Grow Team image/link previews. However, some legacy Dropbox
...
In Grow Team 4.0 (feature level 24), the rarely used `!avatar()`
