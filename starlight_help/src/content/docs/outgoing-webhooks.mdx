...
Outgoing webhooks allow you to build or set up Grow Team integrations
which are notified when certain types of messages are sent in Grow Team.
When one of those events is [triggered](#triggering), the Grow Team server
will send an HTTP POST [payload](/api/outgoing-webhook-payload) to the
webhook's configured URL.

Outgoing webhooks can be used to power a wide range of Grow Team
  ...
  <TabItem label="Desktop/Web">
    <FlattenedSteps>
      ...
      1. Fill out the fields, with **Endpoint URL** set to the URL you'd like
         Grow Team to post to, and the outgoing webhook [format](/api/outgoing-webhook-payload)
         you plan on using.
      1. Click **Add**.
    </FlattenedSteps>
  </TabItem>
...
can be configured by the administrator of the Grow Team server by setting
...
send a reply message into Grow Team.  Grow Team's outgoing webhook API
provides a convenient way to do that by simply returning an
appropriate HTTP response to the Grow Team server.
...
The `content` field should contain [Grow Team-flavored Markdown](/help/format-your-message-using-markdown).

Note that an outgoing webhook bot can use the [Grow Team REST
