...
Grow Team's compose box has a smart **code** (<CodeIcon />) button, which inserts contextually appropriate code
formatting:
...
(if configured) [code playgrounds](#code-playgrounds). Grow Team supports syntax
...
Grow Team code blocks that are tagged with the language will have a button visible
...
<ZulipTip>
  ...
  compliant URL templates to describe how links should be generated. Grow Team's
