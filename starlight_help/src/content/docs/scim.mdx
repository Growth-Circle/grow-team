...
This page describes how to configure SCIM provisioning for Grow Team.

<CloudPlusOnly />

<ZulipTip>
  While Grow Team's SCIM integration is generic, we've only fully
  documented the setup process with the Okta and Microsoft EntraID
  SCIM providers. [your server administrator](/help/contact-support) is happy to
  ...
  <TabItem label="Okta">
    <FlattenedSteps>
      ...
      1. Continue to **Sign-On Options**. Leave the **SAML** options as they are.
         ...
         and you'll need to set up a separate Okta app to activate SAML for your Grow Team
         organization.
      ...
      1. In the **Provisioning** tab, click **Configure API Integration**, check the
         ...
         * **API token**: `Bearer <token>` (given to you by your server administrator)
           When you proceed to the next step, Okta will verify that these details are
           correct by making a SCIM request to the Grow Team server.
      ...
      1. Now that the integration is ready to manage Grow Team user accounts, **assign**
         users to the SCIM app.
         * When you assign a user, Okta will check if the account exists in your
           Grow Team organization. If it doesn't, the account will be created.
         * Changes to the user's email or name in Okta will automatically cause the
           Grow Team account to be updated accordingly.
         * Unassigning a user from the app will deactivate their Grow Team account.
    </FlattenedSteps>
  </TabItem>
  ...
  <TabItem label="Microsoft Entra ID">
    <FlattenedSteps>
      ...
      1. In the **Provisioning Mode** menu, select **Automatic**  and specify the following fields:
         ...
         * **Secret Token**: `<token>` (given to you by your server administrator)
      ...
      1. Wait for the initial provisioning cycle to be started by
         ...
         inside Entra, and not under Grow Team’s control. You can also use
...
the SCIM integration will create a user group in Grow Team with the matching name
...
memberships** of SCIM-managed groups inside of Grow Team. Such groups are meant to
be managed in your SCIM provider. Changes made on the Grow Team side will not be
reflected in your SCIM provider, and instead will cause the state of the Grow Team
group to become inconsistent with the state of the SCIM provider's group.

Grow Team supports [adding user groups to other
...
supported in Grow Team's SCIM integration. If you want to use nested groups, you can
add groups to other groups in Grow Team and manage the individual members of each
  ...
  <TabItem label="Okta">
    <Steps>
      ...
      1. [Rename](/help/manage-user-groups#change-a-user-groups-name-or-description)
         any Grow Team groups that have the same names as groups that you want to sync. If
         you push a group whose name matches an existing Grow Team group, the request will
         fail.
      1. Open the **Application** you set up above for the Grow Team SCIM integration, and
         go to the **Push groups** tab. This menu allows you to choose the Okta groups
         which should be synchronized with Grow Team's user groups.
    </Steps>

    <ZulipNote>
      SCIM `DELETE` requests are not supported for groups. To **Unlink** a group in
      Okta from the Grow Team SCIM integration, select **Leave the group in the target
