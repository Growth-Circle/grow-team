...
<ZulipTip>
  This feature is not available on self-hosted Grow Team servers where
...
You can send emails to Grow Team channels. This can be useful:

* If you have an email that you want to discuss on Grow Team
...
<FlattenedSteps>
  ...
  1. Select **Who should be the sender of the Grow Team messages for this email
     address**.
...
The email subject will become the Grow Team topic, and the email body will
become the Grow Team message.
...
You can configure **who should be the sender of the Grow Team messages** for the
generated email address, with the following options:

* **Email Gateway bot**: This option makes it easy to see that the message
  was sent via email.
* **You**: Messages will look the same as messages you send from the Grow Team UI.
...
* **The sender's email address**: Adds `From: <Sender email address>` to
  the top of the Grow Team message.
* **Email footers**: By default, Grow Team tries to automatically remove some footer
  text (like signatures). With this option enabled, Grow Team will include all footers.
* **Quoted original email**: In many email clients, when you reply to a message
  ...
  automatically added to the bottom of your reply. By default, Grow Team tries
  to remove that copied message. With this option enabled, Grow Team will include it.
* **Use html encoding**: The body of an email is typically encoded using
  ...
  HTML (`text/html`).  Grow Team supports constructing the Grow Team message
  content using either (converting HTML to Markdown for the HTML
  format).  By default, Grow Team will prefer using the plain text version
...
* [Using Grow Team via email](/help/using-zulip-via-email)
