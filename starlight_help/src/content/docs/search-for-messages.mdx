...
It's easy to find the right conversation with Grow Team's powerful search. When you
find the message you were looking for, go directly to its topic for full context.

Whenever you need a reminder of the search filters that Grow Team offers, check out
the convenient [**search filters**](#search-filters-reference) reference
in the Grow Team web and desktop apps.
...
Grow Team lets you search messages and topics by keyword. For example:
...
* Grow Team will find messages containing similar keywords (keywords with the same
  stem), so, e.g., `wave` will match `waves` and `waving`.
* Grow Team search ignores very common words like `a`, `the`, and about 100 others.
...
Grow Team also offers a wide array of search filters, which can be used on their
...
<ZulipTip>
  As you start typing into the search box, Grow Team will suggest search filters
  you can use.
</ZulipTip>
...
Grow Team offers the following filters based on the location of the message.
...
To avoid cluttering your search results, by default, Grow Team searches just the
...
Each message in Grow Team has a unique ID, which is used for [linking to a specific
...
All of Grow Team's search filters can be negated to **exclude** messages matching
...
When you search Grow Team, the URL shown in the address bar of the Grow Team web app is a
...
A summary of the search filters above is available in the Grow Team app.
