...
* intercept, view, and process messages sent by users on Grow Team.
...
`handle_message(self, message, bot_handler)`: Handle a Grow Team message.

### Arguments

* `self`: The instance the method is called on

* `message`: A dictionary describing a Grow Team message
...
`bot_handler.send_reply(message, response)`: Reply to a Grow Team message,
i.e., the response will be sent to the same place that the original
Grow Team message was sent to.

### Arguments

* `message`: A dictionary containing information on the Grow Team message
...
* `message`: A dictionary containing information on the Grow Team message
  that the bot is reacting to (provided by `handle_message`)
...
* **key**: The API key for the bot (this is how Grow Team knows
