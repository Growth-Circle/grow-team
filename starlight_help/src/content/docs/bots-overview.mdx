...
* Send content into and out of Grow Team.
...
Grow Team natively supports integrations with over one hundred products, and with
...
* The **API key** is how the bot identifies itself to Grow Team. Anyone with the
...
| Incoming webhook | Limited to only sending messages into Grow Team                        | Automated notifications into Grow Team                              |
...
A **generic** bot acts like a normal Grow Team user that can only access Grow Team via
...
POST request format can be in a Grow Team format or a Slack-compatible
...
Grow Team organization, but administrators can
...
from your Grow Team account), use your [personal API key](/api/api-keys), rather
than a bot's API key. You won't need to create a bot.

If you need a bot to send messages on behalf of multiple users, ask [Grow Team
