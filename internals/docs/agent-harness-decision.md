...
Status tambahan 2026-09-24: [keputusan SDK agent](agent-sdk-decision.md) mengatur
jalur cepat untuk job `answer` dan `manage`. Halaman ini tetap berlaku untuk jalur
code. Lihat juga [spesifikasi jalur cepat](spec/2026-09-24-agent-fast-lane.md).
...
| Protokol agent   | ACP lewat harness `buzz-acp` dengan 1 sampai 32 proses (`crates/buzz-acp/src/config.rs:298-300`).                                            | Protokol sendiri. ACP sudah dipensiunkan (`packages/services/test/nonCliAcpRetirement.test.ts`).                                               | **[jalur cepat]** `@anthropic-ai/sdk` di proses runner, tanpa ACP. **[jalur code]** ACP (`codex-acp` 1.12.0) dan runtime endpoint. |
...
| 5   | Dipensiunkan 2026-09-24: jalur cepat juga berhenti memakai ACP, jadi ini tidak lagi membedakan ZCode dari Grow Team. Lihat [keputusan SDK agent](agent-sdk-decision.md). | — |
| 6   | ZCode tidak punya sandbox OS. **[jalur code]** Keamanan bergantung pada container dan broker alat. **[jalur cepat]** Keamanan bergantung pada otoritas operasi server: server memutuskan dan menjalankan setiap efek, dan proses runner tidak menjalankan kode. | ZCode `NOTICE.md:9`; [kontrak containment](../../services/grow-agent-runner/CONTAINMENT.md); [jalur cepat](spec/2026-09-24-agent-fast-lane.md) bagian 10. |
...
| --------- | -------------------------------------------------------------------------- | ----- | ------ | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1         | Ganti batas absolut 60 detik dengan timeout idle.                          | ZCode | Tinggi | S      | `model-broker.ts` memutus request model sesudah 60 detik total. `endpoint-child.ts` memutus request alat sesudah 60 detik tanpa aktivitas, padahal shell boleh berjalan 120 detik. Buktikan dengan tes shell 90 detik. **[jalur cepat]** Digantikan oleh `idle_timeout_s` pada `model_policy` di stream SDK (lihat [jalur cepat](spec/2026-09-24-agent-fast-lane.md) bagian 6.3). |
| 2         | Kirim status job secara real-time dan tampilkan agent yang sedang bekerja. | Buzz  | Tinggi | M      | Drawer tugas memakai polling 5 detik. **[jalur cepat]** Masuk jalur cepat lewat event `agent_job_ready` dan `result.draft` (lihat [jalur cepat](spec/2026-09-24-agent-fast-lane.md) bagian 5.1; [streaming](spec/2026-09-24-agent-streaming-delivery.md) bagian 4). |
...
| 9         | Runner dengan beberapa slot.                                               | Buzz  | Tinggi | L      | Runner mengklaim `capacity: 1`, dan batas realm adalah 2 job aktif. **[jalur cepat]** Masuk jalur cepat sebagai kapasitas per jalur: `fast` default 4, `code` tetap 1 (lihat [jalur cepat](spec/2026-09-24-agent-fast-lane.md) bagian 5.2). |
...
| 15        | Dialek `anthropic-messages` dan preset provider.                           | ZCode | Sedang | M      | Codec hanya menerima Chat Completions dan Responses. **[jalur cepat]** `anthropic_messages` menjadi dialek utama lewat `@anthropic-ai/sdk` (lihat [keputusan SDK agent](agent-sdk-decision.md)). **[jalur code]** Codec ACP tetap Chat Completions dan Responses. |
...
| Mode `yolo` dan eksekusi tanpa sandbox                  | ZCode  | Grow Team mewajibkan broker alat dan container. **[jalur code]** Container wajib berlaku untuk jalur code. Jalur cepat tidak memakai container (lihat [keputusan SDK agent](agent-sdk-decision.md)). |
