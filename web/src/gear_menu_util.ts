...
export function version_display_string(): string {
    ...
    if (realm.zulip_version.endsWith("-dev+git")) {
        ...
        return $t({defaultMessage: "Grow Team Server dev environment"});
    }
    ...
    if (is_fork) {
        ...
        const display_version = realm.zulip_merge_base
            ...
            .replace(/(-beta\d+).*/, "$1")
            .replace(/-dev.*/, "-dev");
        return $t({defaultMessage: "Grow Team Server {display_version} (modified)"}, {display_version});
    }
    ...
    if (version.includes("+git")) {
        ...
        return $t({defaultMessage: "Grow Team Server {display_version} (patched)"}, {display_version});
    }

    const display_version = version.replace(/\+git.*/, "").replace(/-dev.*/, "-dev");
    return $t({defaultMessage: "Grow Team Server {display_version}"}, {display_version});
}
